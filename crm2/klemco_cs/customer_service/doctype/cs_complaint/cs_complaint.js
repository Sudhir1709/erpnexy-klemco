...
function suggest_assignee(frm) {
    ...
    const match = ROUTING.find(r => r.type === frm.doc.complaint_type);
    if (!match) return;
    frm.set_value('algorithm_suggested', match.dept);
    frm.set_df_property('assigned_to', 'description',
        'Auto-suggested: ' + match.dept + ' (based on complaint type routing)');
    frappe.show_alert({message: 'Suggested assignee: ' + match.dept, indicator: 'blue'}, 4);
}
